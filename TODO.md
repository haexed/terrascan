...
23. `todo find everything like "✨ No stale tasks found". it's a popup which are blocking, and it's factually wrong, and it looks like magic hurray when it's an error message, just do a normal toast.` — fixed: that specific message (`tasks.js`) was already a non-blocking toast, not the target. Found the real thing: 7 native `alert()` popups on `/system` (Collect All Environmental Data, Collect Biodiversity Data) and `/system/schema` (Refresh Schema/export) — genuinely blocking. Converted all 7 to the same toast pattern, moved `showNotification()` into `base.html` so every page has it (removed the now-duplicate copy from `tasks.js`), delayed the post-success `location.reload()` by 1.5s so the toast is visible before reload. Also found `app.js` and `hero-map.js` are dead — not `<script src>`'d by any template — left alone, not asked to delete.
